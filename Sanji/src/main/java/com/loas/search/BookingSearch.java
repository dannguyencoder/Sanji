...
public class BookingSearch {
   ...
   */
  public List<Booking> search(String text) {
    ...
    QueryBuilder queryBuilder = 
      ...
      .buildQueryBuilder().forEntity(Booking.class).get();
    
    // a very basic query by keywords
    org.apache.lucene.search.Query query =
      queryBuilder
        .keyword()
        .onFields("lastName", "firstName","phone")
        .matching(text)
        .createQuery();

    // wrap Lucene query in an Hibernate Query object
    org.hibernate.search.jpa.FullTextQuery jpaQuery =
      fullTextEntityManager.createFullTextQuery(query, Booking.class);
  
    // execute search and return results (sorted by relevance as default)
    @SuppressWarnings("unchecked")
    List<Booking> results = jpaQuery.getResultList();
